...
# }}}
VCS_INFO_clearcase_get_data () { # {{{
    VCS_INFO_formats '' "${CLEARCASE_ROOT:t}"
    return 0
}
# }}}
...
# }}}
VCS_INFO_clearcase_detect() { #{{{
    [[ -n "$CLEARCASE_ROOT" ]] && return 0
    return 1
}
# }}}
VCS_INFO_cvs_detect() { #{{{
...
vcs_info () { # {{{
    ...
    vcs="init"
    VCSs=(git hg bzr darcs svk mtn svn cvs clearcase cdv tla)
    case $1 in
        (print_systems_)
